...
export const WelcomeDemo = ({ title, description, isAnonymous = false }: WelcomeDemoProps) => {
  ...
  const handleFinishWelcome = async () => {
    if (isAnonymous) {
      // For anonymous users, just save to localStorage and continue
      const nodes = getNodes();
      const edges = getEdges();
      
      console.log('Saving to localStorage and redirecting to canvas...');
      ...
      // Redirect to free-form canvas
      console.log('Calling router.push(/canvas)');
      router.push('/canvas');
      return;
    }
      ...
      action: (
        <div className="not-prose flex items-center gap-4">
          ...
          <Button variant="outline" onClick={handleFinishWelcome}>
            {isAnonymous ? "Skip to canvas" : "Skip intro"}
          </Button>
        </div>
      ),
      complete: started,
          ...
          {isAnonymous ? (
            <>
              ...
              <Button variant="outline" asChild>
                <Link href="/canvas">Continue exploring</Link>
              </Button>
            </>
          ) : (
