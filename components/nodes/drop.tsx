...
export const DropNode = ({ data, id }: DropNodeProps) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Handle numbered hotkeys (1-5 for the filtered node buttons)
      const num = parseInt(event.key);
      if (num >= 1 && num <= 5) {
        const filteredButtons = nodeButtons.filter(
          (button) => button.id !== 'file' && button.id !== 'tweet'
        );
        const selectedButton = filteredButtons[num - 1];
        if (selectedButton) {
          handleSelect(selectedButton.id, selectedButton.data);
        }
      }
    };
  ...
  return (
    <div ref={ref}>
      <NodeLayout id={id} data={data} type="drop" title="Add a new node">
        <Command className="rounded-lg w-[280px]">
