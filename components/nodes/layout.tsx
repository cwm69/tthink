    ...
    <>
      ...
      <ContextMenu onOpenChange={handleSelect}>
        <ContextMenuTrigger>
          ...
          >
            {type !== 'drop' && (
              <div className="-translate-y-full -top-2 absolute right-0 left-0 flex shrink-0 items-center justify-between drag-handle cursor-move">
                <p className="font-mono text-muted-foreground text-sm tracking-tighter uppercase">
                  {title}
                </p>
              </div>
            )}
            <div
              className={cn(
                'node-container flex h-full w-full flex-col divide-y rounded-[28px] bg-card p-2 ring-1 ring-border transition-all',
                className
              )}
            >
              <div 
                className={cn(
                  "overflow-hidden bg-card nodrag cursor-default flex-1 min-h-0",
                  type !== 'code' && 'rounded-3xl'
                )}
              >
                {children}
              </div>
