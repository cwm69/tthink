...
export const MobileNotesView = ({ nodes, edges, projects, currentProjectId }: MobileNotesViewProps) => {
  ...
  const getConnectedNodes = (nodeId: string) => {
    ...
    return connectedEdges.map(edge => {
      ...
      return {
        id: connectedNodeId,
        title: (connectedNode?.data as any)?.title || getNodeTypeLabel(connectedNode?.type || ''),
  ...
  const getNodeContent = (node: Node) => {
    switch (node.type) {
      case 'text':
        ...
        return (node.data as any).text || (node.data as any).instructions || 'No content';
      case 'image':
        return (node.data as any).content?.url ? 'Image attached' : 'No image';
      case 'audio':
        return (node.data as any).transcript || ((node.data as any).content?.url ? 'Audio attached' : 'No audio');
      case 'video':
        return (node.data as any).content?.url ? 'Video attached' : 'No video';
      case 'code':
        const generatedCode = (node.data as any).generated?.text;
        if (generatedCode) {
          return generatedCode;
        }
        // Fallback to instructions if no generated content
        return (node.data as any).instructions || 'No code';
      case 'file':
        return (node.data as any).content?.name || 'No file';
      case 'tweet':
        return (node.data as any).content?.text || 'No tweet content';
      default:
        return 'No content';
    }
  };
  ...
  return (
    <>
      ...
      {nodes.map((node) => {
        ...
        return (
          <Card key={node.id} id={`node-${node.id}`} className="w-full">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {(node.data as any)?.title && (
                    <span className="font-medium text-sm">{(node.data as any).title}</span>
                  )}
                </div>
                {hasLongContent && (
                  <button
        ...
        {nodes.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Switch to desktop to create your first node
            </p>
          </div>
        )}
